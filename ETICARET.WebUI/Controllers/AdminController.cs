...
namespace ETICARET.WebUI.Controllers
{
    public class AdminController : Controller
    {
        ...
        public IActionResult CreateProduct()
        {
            ...
            //Kategorilerin adlarını ve id değerlerini dropdownlistte göstermek için SelectListItem nesneleri oluşturuyoruz.
        ...
        public async Task<IActionResult> CreateProduct(ProductModel model, List<IFormFile> files)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //dosyalar (resimler) kontrol edilir en az 4 resim yüklenmesi gerekmektedir.
                if (files.Count < 4 || files == null)
                {
                    ModelState.AddModelError("", "Lütfen en az 4 adet resim yükleyin");
                    ViewBag.Category = _categoryService.GetAll().Select(x => new SelectListItem { Text = x.Name, Value = x.Id.ToString() });
                    return View(model);
                }
                foreach (var item in files)
                {
                    ...
                    entity.Images.Add(image); //resmi ürünün resim listesine ekliyoruz
                    var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot\\img", item.FileName); //resmin kaydedileceği yolu belirliyoruz
                    using (var stream = new FileStream(path,FileMode.Create))
                    {
                        await item.CopyToAsync(stream);
                    }
                }
                //ürünle kategori arasında ilişki kuruyoruz. Bir ürünün bir kategorisi olabilir.
                entity.ProductCategories = new List<ProductCategory> { new ProductCategory { CategoryId = int.Parse(model.CategoryId),ProductId = entity.Id } }; //ürünün kategorisini belirliyoruz
        ...
        public async Task<IActionResult> EditProduct(ProductModel model, List<IFormFile> files, int[] categoryIds)
        {
            ...
            if (files != null && files.Count > 0)
            {
                ...
                {
                    Image image = new Image();
                    image.ImageUrl = file.FileName; //resmin yolunu kaydediyoruz
                    entity.Images.Add(image); //resmi ürünün resim listesine ekliyoruz
                    var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot\\img", file.FileName); //resmin kaydedileceği yolu belirliyoruz
                    using (var stream = new FileStream(path, FileMode.Create))
                    {
